...
- (void)updateViews {
    ...
    [self.allFriends enumerateObjectsUsingBlock:^(id _Nonnull friend, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([friend isKindOfClass:[KOFriend class]]) {
            ...
            nickName = [friend properties][KOUserNicknamePropertyKey];
        }
...
- (void)requestMe {
    [KOSessionTask meTaskWithCompletionHandler:^(id result, NSError *error) {
        if (error) {
            ...
            [self.allFriends addObject:result];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([anyObject isKindOfClass:[KOUser class]]) {
        ...
        [cell.imageView sd_setImageWithURL:[NSURL URLWithString:user.properties[KOUserThumbnailImagePropertyKey]]
...
- (void)sendTemplateMessage {
    ...
    if ([self.selectedFriend isKindOfClass:[KOUser class]]) {
        KOUser *user = (KOUser *)self.selectedFriend;
        
        [user sendMemoWithTemplateId:MESSAGE_VIEW_CONTROLLER_MEMO_TEMPLATE_ID
